...
func (db *DB) ListTasks(quadrant *Quadrant, status *string, project *string, updatedSince, updatedUntil *time.Time, summary *string) ([]*Task, error) {
	query := `
		SELECT id, title, description, quadrant, priority, status, due_date, created_at, updated_at, finished_at, started_at, tags, progress, summary, project
		FROM tasks
		WHERE 1=1
